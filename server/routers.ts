  ...
  accounts: router({
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (accountDialogs.length > 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Нельзя удалить аккаунт с историей диалогов. Используйте переподключение или отключение, чтобы сохранить сообщения.",
          });
        }
    ...
    debugStatus: protectedProcedure.query(async ({ ctx }) => {
      ...
      const accs = await db.select({
        ...
        lastError: telegramAccounts.lastError,
    ...
    syncProgress: protectedProcedure.query(async () => {
      ...
      const accs = await db.select({
        ...
        syncStatus: telegramAccounts.syncStatus,
        syncedDialogs: telegramAccounts.syncedDialogs,
        totalTgDialogs: telegramAccounts.totalTgDialogs, // real count from Telegram
        lastSyncAt: telegramAccounts.lastSyncAt,
      }).from(telegramAccounts);
      // Count dialogs already saved in CRM per account
      ...
      return accs.map(acc => ({
        ...acc,
        crmDialogs: dialogCountMap.get(acc.id) ?? 0,   // dialogs saved in CRM
        totalTgDialogs: acc.totalTgDialogs ?? 0,         // real total from Telegram
